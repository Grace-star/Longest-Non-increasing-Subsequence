...
bool is_nonincreasing(const sequence& A) {
  ...
  // statement, and then delete these comments.
    ...
    //iterate throuth the sequence to compare each adjacent two number pairs
    //to test if the subsequence of the pair is non-increasing
    for (size_t i = 1; i < A.size(); i++) {
      //if we find an increasing subsequence, return immediately
      if (A[i] > A[i - 1]) return false;
    }

    //if reaching here, we know there is no increasing subsequence in the sequence
    return true;
  }
...
sequence longest_nonincreasing_end_to_beginning(const sequence& A) {
  ...
  for (signed int i = n-2;  i>= 0; i--) {
    for (size_t j = i+1; j < n ; j++) {
      ...
      if (A[i] >= A[j] && H[i] <= H[j]) {
        //update H[i] based in value of H[j]
        H[i] = H[j] + 1;
      }
    }
  }
  ...
  for (size_t i = 0; i < n; ++i) {
    if (H[i] == index) {
      ...
      if (index == max - 1 || A[i] <= R[j - 1]) {
        //when find a new element, add it to result and update value of index and j
        R[j] = A[i];
        j++;
        index--;
      }
...
sequence longest_nonincreasing_powerset(const sequence& A) {
  ...
  while (true) {
    ...
    if (is_nonincreasing(candidate) && candidate.size() > best.size()) {
      //if reach here we know we find a better candidate for longerest non-increasing subsequence, update best
      best = candidate;
    }
  }
